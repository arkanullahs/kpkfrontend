...
const DEFS: { key: Screen; label: string; path: string }[] = [
  { key: "ask", label: "Ask", path: "M4 8h8M17 8h3M4 16h3M12 16h8M14.5 5.5a2.5 2.5 0 100 5 2.5 2.5 0 000-5M7.5 13.5a2.5 2.5 0 100 5 2.5 2.5 0 000-5" },
  { key: "results", label: "Results", path: "M4 6h2M4 12h2M4 18h2M10 6h10M10 12h10M10 18h10" },
  { key: "detail", label: "Detail", path: "M8.5 3.5h7a1.5 1.5 0 011.5 1.5v14a1.5 1.5 0 01-1.5 1.5h-7A1.5 1.5 0 017 19V5a1.5 1.5 0 011.5-1.5zM10.5 17.5h3" },
];
        ...
        {DEFS.map((it) => {
          ...
          return (
            <button key={it.key} onClick={() => onScreen(it.key)}
              ...
              <span style={st("font-size:11px; font-weight:600; letter-spacing:.3px;")}>{it.label}</span>
            </button>
          );
        })}
      </div>
      ...
      {screen === "ask" ? (
        <button onClick={onSeeResults}
          ...
          <span style={st("display:flex; flex-direction:column; align-items:flex-start; line-height:1.2;")}>
            <span style={st("font-size:11px; font-weight:500; opacity:.85;")}>{matchCount != null ? `${matchCount} matches` : "live picks"}</span>
            <span style={st("font-size:15px; font-weight:700;")}>See results</span>
          </span>
        ...
        <button onClick={onAsk} title="New search"
